	...
	function(phy, PIXI, Box2D, animated) {
		...
		function createBox(floor) {
			...
			for (var i = 0; i < floor.width; i ++) {
				var lightDef = new Box2D.Dynamics.b2FixtureDef();
				lightDef.userData = {
					type: 'flare',
					index: i
				};
				lightDef.shape = new Box2D.Collision.Shapes.b2PolygonShape();
				lightDef.shape.SetAsOrientedBox(10 / 28, 10 / 28, 
						new Box2D.Common.Math.b2Vec2(i, -1.7),
						0);
				ground.CreateFixture(lightDef);
			}
